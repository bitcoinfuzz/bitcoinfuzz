...
extern "C" size_t LLVMFuzzerCustomMutator(uint8_t *fuzz_data, size_t size,
  ...
  // Decode the input
  bech32::DecodeResult decoded_result =
      bech32::DecodeNoChecksum(input_str, bech32::CharLimit::CUSTOM_MUTATOR);
  ...
  size_t mutated_size =
      LLVMFuzzerMutate(decoded_data.data(), decoded_data_size, max_size);
...
extern "C" size_t LLVMFuzzerCustomCrossOver(const uint8_t *in1, size_t in1_size,
  ...
  bech32::DecodeResult decoded_result1 =
      bech32::DecodeNoChecksum(input1_str, bech32::CharLimit::CUSTOM_MUTATOR);

  // Decode second input
  std::string_view input2_str(reinterpret_cast<const char *>(in2), in2_size);
  bech32::DecodeResult decoded_result2 =
      bech32::DecodeNoChecksum(input2_str, bech32::CharLimit::CUSTOM_MUTATOR);

  // Perform cross-over on decoded data
  std::vector<uint8_t> crossed_data = cross_over(
      decoded_result1.data, decoded_result2.data, max_out_size, seed);
