...
static bool init_jvm() {
    ...
    bitcoinJWrapperClass = env->FindClass("bitcoinj/BitcoinJWrapper");
    if (!bitcoinJWrapperClass) {
        return false;
    }

    bitcoinJWrapperClass = static_cast<jclass>(env->NewGlobalRef(bitcoinJWrapperClass));

    createMasterKeyMethod = env->GetStaticMethodID(bitcoinJWrapperClass,"createMasterKey","([B)Ljava/lang/String;");
...
static std::optional<std::string> bitcoinj_bip32_master_keygen(std::span<const uint8_t> buffer) {
    if (!init_jvm() || !jvm) {
        return "";
    }
    JNIEnv* env = nullptr;
    ...
    env->SetByteArrayRegion(jSeed, 0, buffer.size(), reinterpret_cast<const jbyte*>(buffer.data()));
    if(!jSeed){
        return "";
    }
    jstring jResult  = static_cast<jstring>(
        env->CallStaticObjectMethod(bitcoinJWrapperClass, createMasterKeyMethod,jSeed )
    );
...
namespace bitcoinfuzz {
    namespace module {
        ...
        std::optional<std::string> BitcoinJ::bip32_master_keygen(std::span<const uint8_t> buffer) const {
