...
call_python_function(const InputType input, size_t input_len,
  ...
  main_module = PyImport_ImportModule("pyhdwallet_main");
  if (!main_module) {
    goto cleanup;
  }
...
char *bip32_master_keygen_pyhdwallet(const uint8_t *data, size_t len) {
...
Pyhdwallet::bip32_master_keygen(std::span<const uint8_t> buffer) const {
  auto result_ptr =
      bip32_master_keygen_pyhdwallet(buffer.data(), buffer.size());
